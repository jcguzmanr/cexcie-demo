...
function SendResultsModal({ open, onClose, careerNames }: { open: boolean; onClose: () => void; careerNames: string[] }) {
  ...
  const [cfg, setCfg] = useState<ComparadorContent | null>(null);
  useEffect(() => {
    fetch("/data/comparador-data.json").then((r) => r.json()).then((j: ComparadorContent) => setCfg(j)).catch(() => {});
  }, []);
    ...
    <Modal open={open} onClose={onClose} title="Enviar resultados">
      {sent ? (
        ...
        <div className="grid gap-4">
          ...
          <div className="flex justify-end">
            <Button onClick={handleSend}>{cfg?.ui.cta.label ?? "Enviar"}</Button>
          </div>
        </div>
      )}
    </Modal>
